            ...
            if (swapPuzzleMode)
            {
                for (int i = 0; i < pieces.Length; i++)
                    if (_filterList == null || (_filterList != null && _filterList.Contains(i)))
                    {
                         ...
                         int swapWithPiece = Random.Range (0, pieces.Length);
                        ...
                        if (randomizeRotation)
                            {
                                pieces[i].transform.RotateAround(pieces[i].renderer.bounds.center, Vector3.forward, rotationSpeed * Random.rotation.z);
                                pieces[swapWithPiece].transform.RotateAround(pieces[swapWithPiece].renderer.bounds.center, Vector3.forward, rotationSpeed * Random.rotation.z);
                            }
                    }
