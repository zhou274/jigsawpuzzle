...
public class GameController_Inspector : Editor 
{
	...
	{   
		...
		if(showGameUI)
		{
			...
			if (game.hintLimit != 0) game.hintCounterUI = EditorGUILayout.ObjectField(new GUIContent("Hints counter", "Link to UI text to visualize remaining hints amount (if hints enabled)"), game.hintCounterUI, typeof(TextMeshProUGUI), true) as TextMeshProUGUI;
			if (game.timer >= 0) game.timerCounterUI = EditorGUILayout.ObjectField(new GUIContent("Time counter", "Link to UI text to visualize remaining time (if timer enabled)"), game.timerCounterUI, typeof(TextMeshProUGUI), true) as TextMeshProUGUI;

			if (game.timer >= 0) game.elapsedTimeUI = EditorGUILayout.ObjectField(new GUIContent("Elapsed time", "Link to UI text to visualize elapsed  time (if timer enabled)"), game.elapsedTimeUI, typeof(TextMeshProUGUI), true) as TextMeshProUGUI;
			game.piecesLeftUI = EditorGUILayout.ObjectField(new GUIContent("Pieces Left", "Link to UI text to visualize pieces left unassembled"), game.piecesLeftUI, typeof(TextMeshProUGUI), true) as TextMeshProUGUI;
